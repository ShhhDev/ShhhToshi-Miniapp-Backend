...
const JWT_SECRET = process.env.JWT_SECRET || (BOT_TOKEN ? BOT_TOKEN + '_shhht_jwt' : 'shhht-dev-secret-change-me');
...
function authMiddleware(req, res, next) {
  // 1) Prefer JWT Bearer (issued after Telegram auth)
  ...
  if (bearer) {
    ...
    if (payload && payload.tid) {
      req.tg = {
        user: {
          id: Number(payload.tid),
          username: payload.username || null
        },
        startParam: null,
        fromJwt: true
      };
      req.isAdminJwt = !!payload.adm;
      return next();
    }
  }

  // 2) Fallback: Telegram WebApp initData (HMAC)
  const initData = req.headers['x-telegram-init-data'] || req.body?.initData;
  const validated = validateInitData(initData);
  if (!validated) return res.status(401).json({ error: 'Invalid or expired auth. Open in Telegram or re-login.' });
  req.tg = validated;
  next();
}
...
function adminMiddleware(req, res, next) {
  ...
  const ok = ADMIN_TELEGRAM_IDS.includes(uid) || !!req.isAdminJwt;
...
(async () => {
  try {
    ...
    app.get('/api/health', async (req, res) => {
      res.json({ ok: true, ts: Date.now(), db: dbx.isPg() ? 'postgres' : 'sqlite', jwt: true });
    });
    app.listen(PORT, () => {
